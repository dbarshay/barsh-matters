...
for (const forbidden of ["POST", "PUT", "PATCH", "DELETE", ".create(", ".update(", ".delete(", ".upsert(", ".createMany(", ".updateMany(", ".deleteMany("]) if (api.includes(forbidden) || page.includes(forbidden)) failures.push("users planning surface must remain read-only; found forbidden write fragment " + forbidden);
