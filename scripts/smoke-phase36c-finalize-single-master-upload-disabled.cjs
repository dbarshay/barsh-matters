...
const PORT = Number(process.env.PHASE36C_PORT || 3322);
...
(async () => {
  console.log("RESULT: Phase 36C disabled single-master upload guard smoke starting");
  console.log("CONTRACT: non-dry-run single-master upload must be blocked without explicit upload/folder/live flags; no upload and no DB mutation");
  ...
  const child = spawn("npm", ["run", "dev", "--", "--hostname", "127.0.0.1", "--port", String(PORT)], {
    ...
    env: {
      ...
      CLIO_SINGLE_MASTER_UPLOAD_REWIRE_ENABLED: "",
  ...
  try {
    ...
    if (res.status !== 403 || !res.json) {
      ...
      throw new Error("Phase 36C expected HTTP 403 JSON disabled-upload response");
    }

    assert(res.json.ok === false, "response ok false");
    assert(res.json.uploadRewired === false, "uploadRewired false while disabled");
    assert(res.json.databaseMutation === false, "databaseMutation false while disabled");
    assert(res.json.noUploadPerformed === true, "noUploadPerformed true while disabled");
    assert(res.json.safety?.noDocumentUploadPerformed === true, "safety confirms no document upload");
    assert(res.json.safety?.noDatabaseRecordsChanged === true, "safety confirms no database mutation");
    assert(res.json.uploadGuard?.uploadRewireEnabled === false, "upload rewire flag false in response");
    assert(res.json.uploadGuard?.createFoldersEnabled === false, "create folders flag false in response");
    assert(res.json.uploadGuard?.liveClioWriteEnabled === false, "live write flag false in response");
    assert(!res.body.includes("fullyUploaded"), "response does not include uploaded document confirmation");
    console.log("PASS: no document upload was performed by disabled single-master upload smoke");
    console.log("PASS: no database mutation was performed by disabled single-master upload smoke");
    console.log("RESULT: Phase 36C disabled single-master upload guard smoke passed");
  } finally {
    child.kill("SIGTERM");
    await wait(500);
