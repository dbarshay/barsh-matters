    ...
    databasePreview: {
      ...
      users: dbUsers.map((user: any) => {
        ...
        return {
          ...
          signerProfileStatus: [user.displayName, user.email, user.phoneExtension, user.faxNumber, user.signatureBlockName].every((value: any) => String(value ?? "").trim().length > 0) ? "Complete" : "Missing Fields",
