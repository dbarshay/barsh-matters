    ...
    const created = await prisma.$transaction(async (tx) => {
      const invoice = await tx.providerClientInvoice.create({
        data: {
          ...
          baseNetRemitToProvider: moneyNumber(totals?.baseNetRemitToProvider),
          costBalanceThisRemittancePeriod: moneyNumber(totals?.costBalanceThisRemittancePeriod),
          costBalanceDeductionCap: moneyNumber(totals?.costBalanceDeductionCap),
          costBalanceAdjustmentToNetRemit: moneyNumber(totals?.costBalanceAdjustmentToNetRemit),
          costBalanceLedgerBefore: moneyNumber(totals?.costBalanceLedgerBefore),
          costBalanceLedgerChange: moneyNumber(totals?.costBalanceLedgerChange),
