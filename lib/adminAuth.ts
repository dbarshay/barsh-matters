...
export type AdminIdentityCookieInput = { id: string; email: string; username: string | null; roleKeys?: string[]; grantedAdminPermissionKeys?: string[]; };
export type BoundAdminIdentityCookie = AdminIdentityCookieInput & { issuedAt: number; source: "owner-username-password"; };
export type AdminSessionIdentityDiagnostics = { authenticated: boolean; identityBound: boolean; id: string | null; email: string | null; username: string | null; roleKeys: string[]; grantedAdminPermissionKeys: string[]; source: "none" | "signed-cookie"; legacyGenericAdminSession: boolean; plannedIdentityCookieName: typeof ADMIN_IDENTITY_COOKIE_NAME; note: string; };
...
function createSignedAdminGateCookieValue(identity?: AdminIdentityCookieInput | null): string {
  ...
  const cleanIdentity = identity && cleanAdminAuthValue(identity.id) && isLikelyAdminEmail(identity.email)
    ? {
        ...
        grantedAdminPermissionKeys: Array.isArray(identity.grantedAdminPermissionKeys) ? identity.grantedAdminPermissionKeys.map(cleanAdminAuthValue).filter(Boolean).sort() : [],
      }
    : null;
    ...
    return {
      ...
      identity: parsed.identity && cleanAdminAuthValue(parsed.identity.id) && isLikelyAdminEmail(parsed.identity.email)
        ? {
            ...
            grantedAdminPermissionKeys: Array.isArray(parsed.identity.grantedAdminPermissionKeys) ? parsed.identity.grantedAdminPermissionKeys.map(cleanAdminAuthValue).filter(Boolean).sort() : [],
          }
        : null,
    };
  } catch {
...
export function createAdminIdentityCookieValue(identity: AdminIdentityCookieInput): string {
  const payload: BoundAdminIdentityCookie = { id: cleanAdminAuthValue(identity.id), email: cleanAdminEmailValue(identity.email), username: cleanAdminAuthValue(identity.username) || null, roleKeys: Array.isArray(identity.roleKeys) ? identity.roleKeys.map(cleanAdminAuthValue).filter(Boolean).sort() : [], grantedAdminPermissionKeys: Array.isArray(identity.grantedAdminPermissionKeys) ? identity.grantedAdminPermissionKeys.map(cleanAdminAuthValue).filter(Boolean).sort() : [], issuedAt: Date.now(), source: "owner-username-password" };
...
export function readSignedAdminIdentityCookie(value: unknown): BoundAdminIdentityCookie | null {
  ...
  try {
    ...
    return { id: cleanAdminAuthValue(parsed.id), email: cleanAdminEmailValue(parsed.email), username: cleanAdminAuthValue(parsed.username) || null, roleKeys: Array.isArray(parsed.roleKeys) ? parsed.roleKeys.map(cleanAdminAuthValue).filter(Boolean).sort() : [], grantedAdminPermissionKeys: Array.isArray(parsed.grantedAdminPermissionKeys) ? parsed.grantedAdminPermissionKeys.map(cleanAdminAuthValue).filter(Boolean).sort() : [], issuedAt: Number(parsed.issuedAt) || 0, source: "owner-username-password" };
  } catch { return null; }
}
...
export function adminSessionIdentityDiagnostics(req: NextRequest): AdminSessionIdentityDiagnostics {
  ...
  return {
    ...
    grantedAdminPermissionKeys: Array.isArray(signedIdentity?.grantedAdminPermissionKeys) ? signedIdentity.grantedAdminPermissionKeys : [],
