...
export const ADMIN_PERMISSION_CATALOG: AdminPermissionCatalogItem[] = [
  ...
  },
  {
    ...
    group: "Emails",
    label: "View Emails",
    description: "View matter/lawsuit email threads and the inbound attachment review queue (read-only).",
    routeScopes: ["/api/graph/thread-sync-preview", "/api/graph/local-thread-preview", "/api/graph/matter-email/unread-count", "/api/graph/inbound-attachments"],
  ...
  },
  {
    ...
    description: "Send and reply to matter/lawsuit email, sync threads from Microsoft Graph, and file inbound email attachments into the matter. Excludes View Only.",
    routeScopes: ["/api/graph/matter-email/send", "/api/graph/thread-sync", "/api/graph/inbound-attachments"],
